  ...
  /**
    ...
    */
    ...
    /**
      ...
      */
    conn.sendButtonMsg = async (jid, text = "", footer = "", buttons = [], quoted = null) => {
    try {
    ...
    let preparedMedia = await prepareWAMessageMedia({ image: { url: imageUrl } }, { upload: conn.waUploadToServer });
        let msg = generateWAMessageFromContent(jid, {
            viewOnceMessage: {
                message: {
                    "messageContextInfo": {
...
conn.sendButtonCard = async (jid, text, footer, cards, quoted = null) => {
  try {
    let preparedCards = await Promise.all(cards.map(async (card) => {
      let preparedMedia = await prepareWAMessageMedia({ image: { url: card.imageUrl } }, { upload: conn.waUploadToServer });
      return {
