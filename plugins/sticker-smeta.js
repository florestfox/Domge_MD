...
const handler = async (m, { conn, text }) => {
	...
	try {
		...
		if (!/webp/.test(mime)) return conn.reply(m.chat, 'Reply sticker!', m)
		let img = await m.quoted.download();
		conn.sendMessage(m.chat, { react: { text: '🕒', key: m.key }})
		if (!img) return conn.reply(m.chat,  'Reply a sticker!', m)
		let img2 = new Image();
		await img2.load(await m.quoted.download());
		stiker = await addExif(img, packname || '© Nanda-Dev', author || '');
	} catch (e) {
		console.error(e);
		if (Buffer.isBuffer(e)) stiker = e;
	} finally {
		if (stiker) conn.sendMessage(m.chat, { sticker: stiker }, { quoted: m });
		else return conn.reply(m.chat,  'Conversion failed', m)
	}
};

handler.help = ["smeta *<reply>*"];
